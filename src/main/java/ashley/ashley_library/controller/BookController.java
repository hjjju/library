...
//    @Autowired//생성자에 autowired 스프링컨테이너에 있는 memberServcie랑 연결해줌
    ...
    public String bookList(Model model,  @RequestParam(required = false, defaultValue = "0", value = "page") int pageNo, @PageableDefault(page = 0,size =10,sort = "title",direction = Sort.Direction.ASC) Pageable pageable) {
...
//        int endPage = Math.min(nowPage + 9, bookList.getTotalPages()); // 전체 페이지보다 많은 경우는 전체 페이지를 반환
        int endPage = Math.min(bookList.getTotalPages(), bookList.getPageable().getPageNumber() + 4);
